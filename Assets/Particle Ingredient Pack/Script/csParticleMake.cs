...
public class csParticleMake : MonoBehaviour
{
    ...
    private float MakeAngle; //Particle Make Angle

    private float StandardAngle; //Standard Angle via ParticleMakeNumber		
    ...
    private void Awake()
    {
        ...
        for (int i = 0; i < ParticleMakeNumber; i++) //Make Particle Object via ParticleMakeNumber
        {
            ...
            Transform Obj = Instantiate(_Particles, transform.position, transform.rotation); // Make Object
            Obj.transform.parent = transform;                                                //Set particle's parent to this root.
